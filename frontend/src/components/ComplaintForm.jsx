...
function ComplaintForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const newComplaint = { id: newComplaintId, category: formData.category, date: new Date().toISOString() };
            recentComplaints.unshift(newComplaint);
            localStorage.setItem('recentComplaints', JSON.stringify(recentComplaints.slice(0, 5)));
        ...
        <div className="max-w-2xl mx-auto bg-white p-6 sm:p-8 rounded-xl shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="5" value={formData.description} onChange={handleTextChange} className="w-full px-4 py-2 border rounded-lg" required></textarea>
                </div>
                <div>
                    <label htmlFor="category" className="block text-sm font-semibold text-gray-700 mb-2">Category</label>
                    <select name="category" value={formData.category} onChange={handleTextChange} className="w-full px-4 py-2 border rounded-lg bg-white">
                ...
                <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <button type="button" onClick={handleUploadClick} className="flex-1 flex items-center justify-center gap-2 bg-slate-100 font-semibold py-2 px-4 rounded-lg">
                            <FaFileUpload /> Upload Files
                        </button>
                        <button type="button" onClick={handleCameraClick} className="flex-1 flex items-center justify-center gap-2 bg-indigo-50 font-semibold py-2 px-4 rounded-lg">
                            <FaCamera /> Use Camera
                        </button>
                    </div>
                    {files.length > 0 && (
                        <div className="mt-3 space-y-2">
                            {files.map((file, index) => (
                                <div key={index} className="flex items-center justify-between bg-gray-100 p-2 rounded-md">
                                    ...
                                    <button onClick={() => removeFile(file.name)} className="text-red-500"><FaTimesCircle /></button>
                                </div>
                            ))}
                        </div>
                    )}
